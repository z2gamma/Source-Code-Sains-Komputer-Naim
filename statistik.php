...
?>
<?php
$kategori = mysqli_real_escape_string($sambungan, $_GET["kategori"]);
if (isset($_GET["pilihkategori"])) {
    if (!empty($kategori)) {
        $sqlstatistik = "SELECT * FROM pelanggan JOIN kategori ON pelanggan.kod_kategori = kategori.kod_kategori WHERE pelanggan.kod_kategori = '$kategori'";
        $querystatistik = mysqli_query($sambungan, $sqlstatistik);
        $rows = mysqli_fetch_assoc($querystatistik);
        $jumlah = mysqli_num_rows($querystatistik);
    } else {
        echo "<script>alert('Sila buat pilihan kategori')</script>";
        $jumlah = "";
        $rows["kategori"] = "";
    }
}
...
<body>
  <div id="wrapper">
    ...
    <div id="body" class="bg-light">
      ...
      <br>
      <div id="statistik">
        <form method="get" name="formkategori" id="formkategori">
          <select type="text" name="kategori" class="form-control">
            <option value="" selected disabled>Sila pilih kategori</option>
            <option value="B">BIASA</option>
            <option value="O">OKU</option>
          </select><br>
          <center><input type="submit" name="pilihkategori" class="btn btn-primary" value="Pilih kategori"></center>
        </form>
        <br>
